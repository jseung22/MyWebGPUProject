...
//// - Atlas: floatplane\textures\particle_atlas.png (다양한 폴백 경로 시도)
//// - 각 큐브: 4x4 아틀라스 타일 랜덤 + 라이트(방향/색/샤이니니스) 랜덤
//// - 어둡게 죽는 픽셀 방지: sRGB 텍스처 + half-Lambert + 조명 바닥값
//// Build: C++17, link GLFW, Dawn(WebGPU) C++ libs
//
//#include <iostream>
//#include <vector>
//#include <cmath>
...
//
//        // random light dir on unit sphere (살짝 아래쪽 성분)
...
//
//        // brighter light color floor (>=0.8)
//        float rcol = 0.8f + 0.2f * u01(rng);
//        float gcol = 0.8f + 0.2f * u01(rng);
//        float bcol = 0.8f + 0.2f * u01(rng);
//        inst.lightColor[0] = rcol; inst.lightColor[1] = gcol; inst.lightColor[2] = bcol;
//
//        // shininess slightly higher baseline
//        inst.shininess = 12.f + 64.f * u01(rng);
//        inst._pad0 = 0.f;
//    }
//}
//
//// ===================== WGSL (half-Lambert + floors) =====================
...
//) -> VSOut {
//  var o : VSOut;
//  let worldPos = inPos + instOffset; // model = identity
//  o.pos = mvp.proj * mvp.view * mvp.model * vec4f(worldPos, 1.0);
//  o.worldPos = worldPos;
...
//  // 경계 블리딩 억제
//  let inset = 0.002;
//  let uvAtlas = uv * (scale - 2.0*vec2f(inset*scale.x, inset*scale.y))
//                + tileOffset + vec2f(inset*scale.x, inset*scale.y);
//
//  let baseColor = textureSample(atlasTex, samp, uvAtlas).rgb;
//
//  // ---- 바닥값 파라미터 ----
//  let ambientMin     = 0.18;   // 전체 기본 밝기
//  let diffuseMin     = 0.12;   // 확산광 바닥
//  let specularBoost  = 0.75;   // 스페큘러 가중
//  let finalMin       = 0.08;   // 최종 색 밑변
//
//  // Lighting (half-Lambert + Phong spec)
//  let N = normalize(normalWS);
//  let L = normalize(-lightDir);
//  let V = normalize(mvp.camPos.xyz - worldPos);
//  let H = normalize(L + V);
//
//  // half-Lambert: 그림자쪽 완전 0 방지
//  let ndotl_hl = max(dot(N,L) * 0.5 + 0.5, 0.0);
//  let diffuse  = max(ndotl_hl, diffuseMin);
//
//  let specular = specularBoost * pow(max(dot(N,H), 0.0), max(shininess, 1.0));
//
//  let color = baseColor * (ambientMin + diffuse * lightColor) + specular * lightColor;
//  let outColor = max(color, vec3f(finalMin));
//
...
//    std::cout << "Searching for atlas texture..." << std::endl;
//    
//    for (auto& c : candidates) { 
//        std::cout << "Trying: " << c << " -> exists: " << fs::exists(c) << std::endl;
//        if (fs::exists(c)) { 
//            path = c; 
...
//    
//    if (path.empty()) { 
//        std::cerr << "Atlas image not found. Tried all paths above.\n"; 
//        std::exit(1); 
//    }
//
...
//
//    std::cout << "Successfully loaded atlas: " << img.w << "x" << img.h << " pixels" << std::endl;
//
//    // ★ sRGB 텍스처로 생성
//    wgpu::TextureDescriptor td{}; td.size = { (uint32_t)img.w,(uint32_t)img.h,1 }; td.mipLevelCount = 1; td.sampleCount = 1;
//    td.dimension = wgpu::TextureDimension::e2D; td.format = wgpu::TextureFormat::RGBA8UnormSrgb;
...
//
//    // ★ sRGB 뷰
//    wgpu::TextureViewDescriptor tvd{}; tvd.dimension = wgpu::TextureViewDimension::e2D; tvd.format = wgpu::TextureFormat::RGBA8UnormSrgb; tvd.mipLevelCount = 1; tvd.arrayLayerCount = 1;
...
//    GLFWwindow* window = glfwCreateWindow(kWidth, kHeight, "Instanced Cubes (Atlas + Per-Instance Lights, Non-Dark)", nullptr, nullptr);
